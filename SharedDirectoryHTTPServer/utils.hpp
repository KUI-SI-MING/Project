...
{
    public:
        static int Divide(std::string& src, const std::string& seg, std::vector<std::string>& list)
        {
            //分割成了多少数据
            int num = 0;
            size_t idx = 0;
            ...
            if (idx < src.length())
            {
                list.push_back(src.substr(idx, pos - idx));                num++;
            }
            return num;
        }
...
class HttpRequest
{
    ...
    public:
        ...
        //接收http请求头
        bool RecvHttpHeader(RequestInformation& info)
        {
            //定义一个设置http头部最大值
        ...
        bool PathIsLeagal(std::string& path, RequestInformation &info)
        {
            //GET / HTTP/1.1
            //file = www/ 
        ...
        bool PraseFirstLine(std::string& line, RequestInformation& info)
        {
            ////请求方法
            //std::string _method;
            ////协议版本
        ...
        //解析http请求头
        bool PraseHttpHeader(RequestInformation& info)
        {
            ...
            PraseFirstLine(hdr_list[0], info);
            //将首行删除
        ...
        //向外提供解析结果
        RequestInformation& GetRequestInfo(); 
};
...
class HttpResponse
{
    ...
    public:
        ...
        //初始化的一些请求响应信息
        bool ResponseInit(RequestInformation req_info)
        {
            //Last-Modified
        ...
        bool IsPartDownload(RequestInformation& info)
        {
            ...
            else 
            {
                ...
                info._part = Utils::Divide(range,", ",info._partlist); 
                return true;
            }
        }
        bool ProcessPartDownload(RequestInformation &info,int i)
        {
        ...
        //文件下载功能
        bool ProcessFile(RequestInformation& info)
        {
        ...
        //文件列表功能
        bool ProcessList(RequestInformation& info)
        {
            //组织头部信息：
            //首行
        ...
        //cgi请求的处理
        bool ProcessCGI(RequestInformation& info)
        {
        ...
        //处理出错响应
        bool ErrorHandler(RequestInformation& info)
        {
            std::string rsp_header;
            std::string rsp_body;
            //首行 协议版本 状态码 状态描述\r\n
        ...
        bool CGIHandler(RequestInformation& info)
        {
            //初始化CGI信息
            ResponseInit(info);
            //执行CGI响应
        ...
        bool FileIsDir(RequestInformation& info)
        {
        ...
        bool FileHandler(RequestInformation& info)
        {
            //初始化文件响应信息
            ResponseInit(info);
            //执行文件列表展示响应
